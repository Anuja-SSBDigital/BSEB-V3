...
public partial class AgencyAccess : System.Web.UI.Page
{
    ...
    private void BindAgencies()
    {
        ...
        {
            if (row["agencyname"] == DBNull.Value ||
                row["AgencyType"] == DBNull.Value)
                continue;

            string agency = row["agencyname"].ToString().Trim();
            string type = row["AgencyType"].ToString().Trim();

            if (string.IsNullOrEmpty(agency) || string.IsNullOrEmpty(type))
                continue;
            ...
            if (!string.Equals(currentType, type, StringComparison.OrdinalIgnoreCase))
            {
                ...
                chkViewerAgencies.Items.Add(
                    new ListItem("<b>" + type + "</b>", "")
                    { Enabled = false });
            }

            chkViewerAgencies.Items.Add(new ListItem(agency, agency));
        }
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
      ...
      //  string examsessontype = ddl_examsession.SelectedItem.Text;

        int sessionId = 0;
        int.TryParse(ddl_examsession.SelectedValue, out sessionId);
            ...
            foreach (string viewerAgency in selectedViewers)
            {
                foreach (string subDoc in selectedDocs)
                {
                    ...
                    using (SqlCommand insertCmd = new SqlCommand(
                      ...
                      (SessionId,OwnerAgency, ViewerAgency, SubDocType, DocType, createddate,doctypeId, IsActive)
                      VALUES
                      (@SessionId,@OwnerAgency, @ViewerAgency, @SubDocType, @DocType, GETDATE(),@doctypeId, 1)", con))
                    {
